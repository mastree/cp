...
int main() {
    ...
    auto can = [&](int L, int R, int A, int B, long long S, long long len) -> bool {
        int l = A, r = B, at = -1;
        // while (l <= r) {
        //     int mid = (l + r) >> 1;
        //     cL = mid; cR = B;
        //     if (getQuery(psegtree[L - 1], psegtree[R], 1, N).first >= len) {
        //         l = mid + 1;
        //         at = mid;
        //     } else {
        //         r = mid - 1;
        ...
        cL = at; cR = B;
        pll cur = getQuery(psegtree[L - 1], psegtree[R], 1, N);
        long long resCnt = cur.first;
        long long sum = cur.second;
        if (resCnt > len) {
