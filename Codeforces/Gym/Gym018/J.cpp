...
int main() { 
    ...
    for (auto& e : mp) {
        ...
        int w = n + cnt;
        dinic.add_edge(u, w, e.second);
        dinic.add_edge(v, w, e.second);
        dinic.add_edge(w, sink, e.second);
        ++cnt;
    }
    ...
    for (int i = 2 * n; i < (int) dinic.edges.size(); i++) {
        FlowEdge& edge = dinic.edges[i];
        if (edge.u == sink || edge.flow != 1) continue;
        int u = edge.v, v = edge.u - n - 1;
        if (u > n || v >= pm.size()) continue;
        int w = pm[v].first;
