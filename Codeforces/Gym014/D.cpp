...
long long queryLarge(int u, int v, int k) {
    int w = LCA(u, v);
...
long long querySmall(int a, int b, int x){ // query for x < sqrt(T_MAX) (harus ditambah node di pathnya)
    long long ret = dep[a] + dep[b] - 2 * dep[LCA(a, b)] + 1;
    for (; head[a] != head[b]; b = up[head[b]][0]){
        if (dep[head[a]] > dep[head[b]]) swap(a, b);
        ret += (1LL) * (pre[pos[b]][x] - pre[pos[head[b]] - 1][x]);
    }
    if (a == b) return ret;
