...
def is_common(word):
    ...
    while (l < r):
        m = (l + r + 1) // 2
        if (commonwords[m] <= word):
            l = m
        else:
            r = m - 1
    return commonwords[l] == word
...
def calc(title, text):
    ...
    titles = [elem for elem in titles if not is_common(elem)]
...
if __name__ == '__main__':
    ...
    with open('stopwords.txt', 'r') as data:
        commonwords = data.read().split('\n')
